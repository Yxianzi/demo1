...
def lmmd(source, target, s_label, t_label, kernel_mul=2.0, kernel_num=5, fix_sigma=None, CLASS_NUM=7, BATCH_SIZE=32):
    ...
    if torch.is_tensor(weight_ss):
        weight_ss = weight_ss.to(source.device)
        weight_tt = weight_tt.to(source.device)
        weight_st = weight_st.to(source.device)
    else:
...
def weighted_lmmd(source, target, s_label, t_prob, t_weight=None, kernel_mul=2.0, kernel_num=5, fix_sigma=None, CLASS_NUM=7, BATCH_SIZE=32):
    batch_size = source.size()[0]
    zero_loss = source.new_tensor(0.0)
    if batch_size == 0:
        return zero_loss
    ...
    s_label = s_label.to(device=device, dtype=torch.long).view(-1)
    t_prob = t_prob.detach().to(device=device, dtype=dtype)

    if t_weight is not None:
        t_weight = t_weight.detach().to(device=device, dtype=dtype).view(-1)
        t_prob = t_prob * t_weight.view(-1, 1)

    effective_target = t_prob.sum(dim=1) > eps
    if torch.sum(effective_target).item() == 0:
        return zero_loss
    t_sca_label = t_prob[effective_target].max(1)[1]
    t_present = F.one_hot(t_sca_label, num_classes=CLASS_NUM).sum(dim=0) > 0
    t_sum = t_prob.sum(dim=0, keepdim=True)
    t_safe_sum = torch.where(t_sum > eps, t_sum, torch.ones_like(t_sum))
    t_vec_label = t_prob / t_safe_sum
    t_vec_label = torch.where(t_sum > eps, t_vec_label, torch.zeros_like(t_vec_label))

    s_onehot = F.one_hot(s_label, num_classes=CLASS_NUM).to(device=device, dtype=dtype)
    s_sum = s_onehot.sum(dim=0, keepdim=True)
    s_present = s_sum.view(-1) > 0
    s_safe_sum = torch.where(s_sum > 0, s_sum, torch.ones_like(s_sum))
    s_vec_label = s_onehot / s_safe_sum

    common_class = (s_present & t_present).to(dtype=dtype)
    class_count = common_class.sum()
    if class_count.item() == 0:
        return zero_loss

    s_vec_label = s_vec_label * common_class.view(1, CLASS_NUM)
    t_vec_label = t_vec_label * common_class.view(1, CLASS_NUM)

    weight_ss = torch.mm(s_vec_label, s_vec_label.t()) / class_count
    weight_tt = torch.mm(t_vec_label, t_vec_label.t()) / class_count
    weight_st = torch.mm(s_vec_label, t_vec_label.t()) / class_count

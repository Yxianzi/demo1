...
from da_lmmd_utils import DistributionCalibrator, blend_target_probability

USE_DA_LMMD_V16 = True
DA_WARMUP_EPOCHS = 20
DA_LMMD_BLEND_MAX = 0.3
DA_PRIOR_MOMENTUM = 0.9
DA_TAU = 0.3
DA_EPS = 1e-6
RP_EVAL_INTERVAL = 10
...
for iDataSet in range(nDataSet):
    ...
    if USE_DA_LMMD_V16:
        target_calibrator = DistributionCalibrator(
            num_classes=CLASS_NUM,
            momentum=DA_PRIOR_MOMENTUM,
            tau=DA_TAU,
            eps=DA_EPS,
            device="cuda",
        )
    ...
    for epoch in range(1, epochs + 1):
        ...
        for i in range(1,num_iter):
            ...
            rho = 0.0
            calibrated_prior = torch.ones(CLASS_NUM).float() / CLASS_NUM

            if USE_DA_LMMD_V16:
                calibrated_prior = target_calibrator.prior.detach().cpu()

            if USE_DA_LMMD_V16 and epoch > DA_WARMUP_EPOCHS:
                calibrated_prob_t = target_calibrator.update_and_calibrate(student_prob_t.detach())

                da_progress = min(
                    1.0,
                    max(0.0, (epoch - DA_WARMUP_EPOCHS) / max(1, epochs - DA_WARMUP_EPOCHS))
                )
                rho = DA_LMMD_BLEND_MAX * da_progress

                target_prob_for_lmmd = blend_target_probability(
                    student_prob_t.detach(),
                    calibrated_prob_t.detach(),
                    rho
                )
        ...
        prior_list = [round(float(x), 4) for x in calibrated_prior.tolist()]
        print('epoch {:>3d}:   cls loss: {:6.4f},lmmd loss:{:6f},con_s loss:{:6f}, con_t loss:{:6f}, rho:{:6.4f}, acc {:6.4f}, total loss: {:6.4f}, student hist:{}, target prior:{}'
              .format(epoch , cls_loss.item(),lmmd_loss.item(),contrastive_loss_s.item(),contrastive_loss_t.item(),
               rho,total_hit / size,loss.item(),student_hist.tolist(),prior_list))
